...
def smart_generate_cover_letter():
    ...
    try:
        db = JobDatabase()
        print(f"💾 Saving cover letter to DB: {db.db_path}")
        ...
        conn = sqlite3.connect(db.db_path)
        cursor = conn.cursor()
        cursor.execute('SELECT COUNT(*) FROM cover_letters WHERE job_id = ?', (job_data['id'],))
        saved_count = cursor.fetchone()[0]
        conn.close()

        print(f"✅ Cover letter saved (ID: {cover_letter_id})")
        print(f"📦 Cover letters for job {job_data['id']}: {saved_count}")
